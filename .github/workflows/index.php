...
if (!isset($_GET['pg'])) {
  ...
  switch ($_GET['pg']) {
    ...
    case 'forgot-password':
      ...
      if (isset($_POST['guiemail'], $_POST['fg_usr'], $_POST['fg_mail'])) {
        ...
        if ($checkmail && is_array($checkmail)) {
          ...
          $context = "https://BarberShop/index.php?pg=reset-pass&id=" . $checkmail['id'];
          ...
          if (isUsernameExists($username)) {
            ...
            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
              ...
              if ($password != $repassword) {
                ...
                $tbdk = "Đăng ký thành công check mail!";
                $usr = $username;
                ...
                if ($checkmail && is_array($checkmail)) {
                  $_SESSION['reset_user_id'] = $checkmail['id'];
                  //Nội dung mail là link dẩn tới trang thay đổi password và có username của user muốn thay đổi pass
                  $context = "http://localhost/barbershop/index.php?pg=verify&id=" . urlencode($checkmail['id']) . "&username=" . urlencode($username) . "&password=" . urlencode($password);
                  ...
                  sendMail($Mailer, "Xac thuc email", $link_text);
                };
              }
            }
          }
    ...
    case 'services':
      ...
      $sosp = 16;
      $dssp = get_dssp_services($kyw, $iddm, $pg, $sosp);
      $tongsp = get_dssp_all_services();
      $hienthist = hien_thi_st_services($tongsp, $sosp);
    ...
    case 'addcart':
      if (isset($_POST["btnaddcart"])) {
        ...
        $sp = array("id" => $id, "name" => $name, "img" => $img, "price" => $price, "amount" => $amount, "thanhtien" => $thanhtien, "appointment_date" => $appointment_date);

        // Clear the cart and add the new product
