...
{
    // Generate a random 60-character user ID
    $randomId = bin2hex(random_bytes(30)); // 30 bytes = 60 characters in hexadecimal format

    // Thực hiện quá trình đăng ký khi tài khoản không tồn tại
    $sql = "INSERT INTO users(id, username, password, email, status) VALUES (?, ?, ?, ?, ?)";
    pdo_execute($sql, $randomId, $username, $password, $email, 1);
}
